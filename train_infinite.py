...
from env_platform_infinite import Platform2DEnv
...
def train_agent(
    ...
    render_every: int = 10,
    save_every: int = 100,
    log_dir: str = "logs_infinite",
    model_dir: str = "models_infinite",
    device: str = "cpu",
    lr: float = 3e-4,
    gamma: float = 0.99,
    eps_clip: float = 0.2,
    ...
    env = Platform2DEnv(render_mode="human", render_every=render_every)
    ...
    for episode in range(episodes):
        ...
        if episode % save_every == 0 and episode > 0:
            model_path = os.path.join(model_dir, f"ppo_agent_episode_{episode}.pth")
            agent.save(model_path)
            print(f"Model saved to {model_path}")
    
    # Final save
    final_model_path = os.path.join(model_dir, "ppo_agent_final.pth")
