"""
2D Platform RL Environment with Food Proximity Sensing.
...
DIFFERENCES:
1. REQUIRES ACTION_EAT - agent must explicitly eat food (no auto-eat)
2. Includes food proximity sensing in observations

This makes it easier to find food (sensing) but still requires learning
the ACTION_EAT behavior.
"""
...
class Platform2DEnvSensing(gym.Env):
    """
    ...
    def __init__(self, render_mode: Optional[str] = None, render_every: int = 10, 
        ...
        if self.render_mode == "human" and PYGAME_AVAILABLE:
            try:
                ...
                pygame.display.set_caption("2D Platform RL Environment (Food Sensing)")
